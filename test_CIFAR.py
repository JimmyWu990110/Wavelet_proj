...
for j, data in enumerate(test_loader, 0):
    ...
    for i in range(len(images)):
        ...
        cv2.imwrite(os.path.join(original_path, str(counter)+".png"), image)
        cv2.imwrite(os.path.join(noisy_path, str(counter)+".png"), noisy_image)
        cv2.imwrite(os.path.join(denoised_path, str(counter)+".png"), output)
        counter += 1
    if j >= 7:
        break
